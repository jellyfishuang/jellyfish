...
Multi-lane 並行 brief：全域單 active brief → scope-based lock registry。動因：使用者常在不同產線（SGC 內不同 `SGC_*` repo 群）同時有 task，被單一 `_active.yaml` mutex 序列化——SGC inbox 積 15 個排隊 brief（各自宣告 affected_repos 且互不依賴），甚至出現手工 `_active.snapshot.yaml` park 插隊的零規範 workaround。改制核心：「1 main session = 1 control plane = 1 root brief」不變量保留（並行單位是 session，不是單 session 多 brief——main context 稀缺性同 e2r-tree §1.2 禁 L2 的理由），變的是併發語意「全域單 active」→「**每 scope 單 active**」；phase 1 粒度 = repo-disjoint only（同 repo 並行需 per-repo worktree，留 phase 2）。minor bump。

- **`core/batch-lock.md`**（重寫）：lock registry `briefs/_active/{brief_id}.yaml`（每 lane 一份，檔名=brief_id，新增 `affected_repos` 冗餘欄 + `scope_status: provisional|confirmed`）；admission 閘（§3.1，`scope_check --overlap` 正式接線——0.9.0 起僅寫在 §2.2 一處、指令檔從未接）；two-phase scope lock（brief-new 預估上鎖 → brief-approve 以 plan 權威值收斂重驗 `--self` 排除自己 → execute 擴 scope 再驗）；lane 互不侵犯紀律（§3.3）；**local_test 全域互斥**（§3.4，integration-tester 起真 service 共享機器資源，repo-disjoint 擋不住——同時僅一條 lane 可處於 local_test phase）；收尾互斥 `_closing.lock`（§3.5）；per-lane 殭屍偵測/recover/unlock（§4）；多 session 由「不支援」升格正式模型（§6）；legacy 單檔遷移節（§8，含 `_active.snapshot.yaml` park 慣例正式退役）
- **`scripts/scope_check.py`**：`active_allowed()` → `active_locks()`（glob registry + legacy 單檔兼容；lock 冗餘欄為主、brief 目錄解析 fallback）；`--overlap` 交集對象改（∪ 他 lane repos ∪ 無主 dirty）+ 新 `--self <brief_id>` 排除自鎖；**dirty 三分類**（屬 allowed=OK / 屬他 lane lock=INFO 不違規 / 無主=VIOLATION exit 2——原邏輯會把他 lane 合法 dirty 誤判違規）；`--self` 單獨用=以該 lane 為 allowed；多 lane 無參數=僅報現況+歸屬
- **`scripts/scope_check_tests.py`**（新增，15 case）：本檔原是 scripts 中唯一無測試者、而本次改的正是它——disjoint 過/撞 lane 歸屬/撞無主 dirty/--self 排除/他 lane dirty=INFO/legacy 兼容/缺欄 fallback/單 lane 無參數/多 lane 報現況/_closing.lock 非 lane/空值 exit 1
- **`scripts/brief_close.py`**：刪鎖改 unlink `_active/{brief_id}.yaml`（legacy 單檔相符也刪，不符保留+警告照舊）；新增 **close-mutex**（`_active/_closing.lock` O_EXCL，stale >10min 搶佔）——多 lane 同時收尾會交錯寫共用 `memory/telemetry/*.jsonl`（檢查鏈內 telemetry_extract 即寫入），收尾全程序列化
- **`scripts/brief_close_tests.py`**：fixture 改 registry 鎖 + 補 T5b（legacy 相符即刪）/T10（close-mutex 擋 + stale 搶佔）→ 21 case；**順修 test debt**：SESSION_MD fixture 補 `draft_cycles`/`fork_count`（0.14.1 session_check 加鍵時未跟上，HEAD 上 11 case 一直 fail 無人察覺）
- **`hooks/gate_mandate.py`**：迭代 registry（+ legacy 兼容），**任一 lane mandate active 即回報**（聯集語意，保守 over-deny）；回傳值 bool → 肇事 `brief_id` 字串（truthy 相容）供 deny 訊息點名；新 `FRAMEWORK_BRIEF_ID` env 精確歸屬（多 lane 並行時他 lane 的 mandate 不誤傷本 session 的 ask 語意——已知副作用：不設時 lane A 掛 mandate 會把 lane B 的 learning loop 合法 memory 寫入從 ask 升 deny）；never-raise 契約不變、單份壞 lock 不拖垮他 lane 判定
- **`hooks/bash_gate.py` / `path_gate.py`**：mandate deny 訊息點名肇事 lane + 指路（consume 該 mandate 或設 `FRAMEWORK_BRIEF_ID`）
- **`hooks/run_tests.py`**：84 → 91 case——fixture 改 registry 型（fakeroot 哨兵同步）+ 新 7 case（legacy 單檔 deny / 雙 lane 聯集 deny×2 / 全非 active ask / env 歸屬他 lane ask / env 命中自 lane deny / 僅 _closing.lock ask）；runner 隔離 `FRAMEWORK_BRIEF_ID` 並支援 per-case env 注入
- **`core/control-plane.md`**：Step A（admission 閘 + registry 建鎖）、Step D（approve scope 收斂重驗）、Step F2（local_test 互斥前置）、Step H/I（per-lane 解鎖）、§1.1/§1.3/§2.1（狀態檔路徑 + 他 lane read-only）、§5.5/§5.6.1（lock phase / mandate 聯集註記）、§8.8（自驗路徑改 registry）
- **接線**：`commands/brief-new.md`（前置條件改 admission 閘 + 新 Step 2.5 + Step 3 建 registry 鎖含自驗）；`brief-status.md`（重寫為跨 lane dashboard：等你的關卡 / 自主推進中 / 產線占用 / inbox 可開案——多 lane 下使用者注意力是 scheduler，此視圖回答「該看哪條線」）；`brief-approve.md`（brief 解析規則 + 行為 1.5 scope 收斂重驗）；`brief-cancel/reopen/amend/import.md`（解析規則 + registry 路徑；reopen/import 走 admission 閘）；`framework-recover/unlock.md`（per-lane 化，unlock 必帶 brief_id）；`framework-status/recall/trust-set.md`（registry 路徑）；`core/escalation-rules/learning-loop/trust-modes/amendment/e2r-tree.md`（路徑 + learning-loop 新增「多 lane append lessons 前重讀檔尾」規則 + trust-modes 白名單改 `_active/*.yaml` 通配）；`init/generator.md`（§2.11 建 `_active/` + .gitignore 改目錄型）；`init/claude-md-template.md`；roles ×4 禁寫清單；`hooks/README.md`；`design-summary.md` §11.3 + 鐵律行加「已被取代」註記（依史料政策不重寫）；根 README
- **同批 back-port**（防 re-sync 蓋掉線上修正）：SGC 部署層三檔超前版回流 lib——`scripts/verdict_check.py`（LEGACY_SCHEMA_CUTOFF mtime grandfather + 雙語序 verdict 檔名）、`scripts/telemetry_extract.py`（`{gate}.waived.json` 豁免檔 + `normalize_checks()` 防炸 + 雙語序）、`hooks/fullwidth_gate.py`（`_written_lines()` 只掃本次寫入行 + stale fail-open）+ `hooks/test_fullwidth_gate.py`（新收錄，13 case）
- 設計來源：D:\Claude 本 repo 2026-09-01 multi-lane 設計對話（研究 → 兩決策拍板：session 模型 B / repo-disjoint only → 全量 touchpoint 稽核 ~59 檔）
