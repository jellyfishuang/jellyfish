...
| `gate_mandate.py` | （共用模組，非 hook） | 判定 user-away mandate 是否生效：迭代 `briefs/_active/*.yaml` lock registry（multi-lane，2026-09-01；legacy 單檔 `_active.yaml` 兼容），各 lock 的 `brief_id` + `autonomous_mandate` 指針 → `_mandate.json.status == "active"`——**任一 lane 生效即回報**（聯集語意，保守 over-deny），回傳生效中的 `brief_id`（truthy 字串）供 deny 訊息點名肇事 lane；多 lane 並行時本 session 可設 `FRAMEWORK_BRIEF_ID` 只看自己 lane 的 lock。動機：永不可預授權的動作（control-plane §5.6.3）不該對不在場的人彈 ask（會 hang 整條線），deny 的 stderr 讓 agent 換路續跑並記入 on_stop.report。任何讀取/解析失敗回 falsy（回退 ask）；`GATE_BRIEFS_DIR` 環境變數覆寫 briefs 目錄（測試 fixture 注入用）；gate 端 import 失敗亦回退 ask |
...
- 回歸測試：同目錄 `run_tests.py`（91 case，含三輪對抗式驗證的全部回饋案例 + mandate ask 升級 deny + multi-lane lock registry（聯集/legacy 兼容/`FRAMEWORK_BRIEF_ID` 精確歸屬）；fixture 於執行時自建於 temp，case 以 `GATE_BRIEFS_DIR` 與真實 repo 隔離，另有 fakeroot 哨兵 case 驗 `__file__` 相對預設路徑與 import 失敗回退）。改任一 gate script 後必跑：`python run_tests.py`，期望 FAILURES: 0。
