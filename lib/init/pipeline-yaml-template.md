...
# ============================================================
# Stage 存在理由標注慣例（2026-07-06，落地時填寫）
# 每 stage 加註解「補償什麼模型弱點 + 本部署遙測線索」。換模型世代時逐條問
# 「這弱點在新模型還成立嗎」——成立留、不成立改條件式或砍（需遙測數據 + 使用者拍板）。
# 遙測: telemetry_report.py；範例見 SGC 部署實例 pipeline.yaml（0.12.0）。
# ============================================================
